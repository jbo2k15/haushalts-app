...
- [ ] **Fairness-Transparenz in der Ruhmeshalle** (erweiterte Variante mit Gewichtung) — Recherche zu Erwachsenen-/Paar-Chore-Apps (evenus.app, FairShare, Chore Chores, FairChore) zeigt: die Ruhmeshalle optimiert aktuell auf Wettbewerb (Pokale, Sieger), nicht auf Fairness (wer trägt wie viel bei). Idee: zusätzliche Karte auf der Ruhmeshalle-Seite, unterhalb der bestehenden Legende:
  ...
  - **Trend-Vergleich zum Vormonat** muss nach derselben gewichteten Formel berechnet werden (gewichteter Anteil des vorherigen 30-Tage-Fensters), nicht nach reiner Erledigungs-Anzahl — sonst sind aktueller und vorheriger Wert nicht vergleichbar.
  ...
  - Mockups wurden in einer Konversation mit Claude Code erstellt (2026-07-09, aktualisiert): (1) Fairness-Karte mit gestapeltem Balken (Lila/Teal), zwei Stat-Kärtchen "erledigt + Ø Schwierigkeit" pro Person, 4-Wochen-Verlauf als Mini-Balkendiagramm; (2) Verwaltungs-Bereich mit Ein/Aus-Schalter "Fairness-Statistik anzeigen" plus Einstieg zum Pflegen der Schwierigkeitsgrade; (3) Aufgabenverwaltung — Aufgabenliste mit Schwierigkeits-Badge und Bearbeiten-Formular mit neuem Leicht/Mittel/Aufwändig-Feld (Default Mittel). Alle visuell im bestehenden App-Stil (abgerundete Karten, gleiche Farbpalette).
