...
// Abfall-Termine (auto-generierte "once"-Aufgaben) werden bis zu einer Woche
// im Voraus angelegt (waste-calendar.js), aber gestaffelt angezeigt: ab
// WASTE_INFO_LEAD_DAYS Tagen vor dem Faelligkeitstag (= Tag vor der Abholung)
// als reine Vorab-Info (nicht abhakbar), ab dem Faelligkeitstag normal
// abhakbar. 2 Tage vor dueDate entspricht 3 Tagen vor der Abholung.
export const WASTE_INFO_LEAD_DAYS = 2
...
export async function getTaskOverview() {
  ...
  for (const task of tasks) {
    ...
    if (task.type === 'once') {
      ...
      // Auto-generierte Abfall-Termine werden vorab "gebankt", aber erst ab
      // WASTE_INFO_LEAD_DAYS vor dem Faelligkeitstag angezeigt: davor gar nicht
      // (nur in der DB), dann als Vorab-Info (infoOnly, nicht abhakbar), ab dem
      // Faelligkeitstag normal. Nutzer-eigene once-Aufgaben: unveraendert.
      let infoOnly = false
      if (task.isAutoGenerated) {
        const infoStart = addDaysToDateString(task.dueDate, -WASTE_INFO_LEAD_DAYS)
        if (today < infoStart) continue // gebankt, aber noch nicht anzeigen
        infoOnly = today < task.dueDate
      }
      const completion = infoOnly ? null : (byKey.get(`${task.id}-${task.dueDate}`) || null)
      result.once.push({
        ...task,
        ...
        isOnce: true,
        infoOnly,
        isOverdue: !completion && !infoOnly && task.dueDate < today,
      })
      continue
    }
...
export async function completeTask(id, { userId, userName }) {
  ...
  // Vorab gebankte Abfall-Termine sind bis zum Tag vor der Abholung nur
  // Vorab-Info (infoOnly, siehe getTaskOverview) und duerfen noch nicht
  // abgehakt werden — auch nicht ueber einen direkten API-Aufruf.
  if (task.type === 'once' && task.isAutoGenerated && task.dueDate && today < task.dueDate) {
    throw httpError(400, 'Dieser Termin kann erst am Tag vor der Abholung erledigt werden')
  }
...
export async function exportTasks() {
  ...
  const pauseMap = await getIndividualPausesForTasks(tasks.map(t => t.id))
  const tasksByType = await prisma.task.findMany({
    where: { type: 'monthly', isAutoGenerated: false },
    select: { id: true, monthlyInterval: true },
  })
  const monthlyIntervalMap = new Map(tasksByType.map(t => [t.id, t.monthlyInterval]))
  return tasks.map(({ id, ...t }) => ({
    ...t,
    weekdays: t.weekdays ? JSON.parse(t.weekdays) : null,
    monthlyInterval: t.type === 'monthly' ? (monthlyIntervalMap.get(id) || null) : null,
...
export async function importTasks(tasks, userId) {
  ...
  const created = await prisma.$transaction(valid.map((t, i) => {
    const data = {
      title: t.title.trim(),
      type: t.type,
    ...
    if (t.type === 'monthly') {
      data.monthlyInterval = Number.isInteger(t.monthlyInterval) ? t.monthlyInterval : null
...
export async function createTask(body, userId) {
  ...
  if (type === 'monthly') {
    data.monthlyInterval = Number.isInteger(monthlyInterval) ? monthlyInterval : null
    data.nextDueMonth = currentMonth
  }
...
export async function updateTask(id, body, userId) {
  ...
  if (type === 'monthly') {
    data.monthlyInterval = Number.isInteger(monthlyInterval) ? monthlyInterval : null
    if (!task.nextDueMonth) data.nextDueMonth = currentMonth
  }
