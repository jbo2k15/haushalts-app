...
export function validateTaskInput({ title, type, priority, weekdays, fixedWeekday, fixedDayOfMonth, dueDate, allowMultiple, weatherDependent }) {
...
export async function listAdminTasks() {
  ...
  return tasks.map(t => ({ ...t, weekdays: t.weekdays ? JSON.parse(t.weekdays) : null }))
}
...
export async function exportTasks() {
  const tasks = await prisma.task.findMany({
    ...
    orderBy: [{ sortOrder: 'asc' }],
    select: { title: true, type: true, priority: true, weekdays: true, fixedWeekday: true, fixedDayOfMonth: true, dueDate: true, isActive: true, allowMultiple: true, weatherDependent: true },
  })
  return tasks.map(t => ({ ...t, weekdays: t.weekdays ? JSON.parse(t.weekdays) : null }))
}
...
export async function createTask(body) {
  const { title, type, priority, weekdays, fixedWeekday, fixedDayOfMonth, dueDate, isActive, allowMultiple, weatherDependent } = body
  const err = validateTaskInput({ title, type, priority, weekdays, fixedWeekday, fixedDayOfMonth, dueDate, allowMultiple, weatherDependent })
