...
import { todayString, addDaysToDateString } from '../../src/lib/dates.js'
...
describe('Abfall-Termine: Vorab-Banking + gestaffelte Anzeige', () => {
  const findOnce = (res, title) => res.body.once.find(t => t.title === title)

  it('zeigt einen weit vorab gebankten Termin (>3 Tage vor Abholung) noch nicht an', async () => {
    const user = await createUser()
    // dueDate = heute+5 -> infoStart = heute+3 -> heute < infoStart -> versteckt
    await createTask({ title: 'Papiertonne rausstellen', type: 'once', dueDate: addDaysToDateString(todayString(), 5), isAutoGenerated: true })
    const res = await request(app).get('/api/tasks').set(authHeader(user.id))
    expect(findOnce(res, 'Papiertonne rausstellen')).toBeUndefined()
  })

  it('zeigt den Termin ab 3 Tagen vor Abholung als Vorab-Info (infoOnly), nicht abhakbar', async () => {
    const user = await createUser()
    // dueDate = morgen -> infoStart = gestern -> heute >= infoStart, heute < dueDate
    await createTask({ title: 'Papiertonne rausstellen', type: 'once', dueDate: addDaysToDateString(todayString(), 1), isAutoGenerated: true })
    const res = await request(app).get('/api/tasks').set(authHeader(user.id))
    const t = findOnce(res, 'Papiertonne rausstellen')
    expect(t).toBeTruthy()
    expect(t.infoOnly).toBe(true)
  })

  it('zeigt den Termin ab dem Fälligkeitstag normal (abhakbar) an', async () => {
    const user = await createUser()
    await createTask({ title: 'Papiertonne rausstellen', type: 'once', dueDate: todayString(), isAutoGenerated: true })
    const res = await request(app).get('/api/tasks').set(authHeader(user.id))
    const t = findOnce(res, 'Papiertonne rausstellen')
    expect(t).toBeTruthy()
    expect(t.infoOnly).toBe(false)
  })

  it('lässt nutzer-eigene once-Aufgaben unverändert sofort sichtbar (kein Banking-Gating)', async () => {
    const user = await createUser()
    await createTask({ title: 'Konzertkarten kaufen', type: 'once', dueDate: addDaysToDateString(todayString(), 5), isAutoGenerated: false })
    const res = await request(app).get('/api/tasks').set(authHeader(user.id))
    const t = findOnce(res, 'Konzertkarten kaufen')
    expect(t).toBeTruthy()
    expect(t.infoOnly).toBe(false)
  })

  it('lehnt das Abhaken eines noch nicht fälligen (Vorab-Info) Abfall-Termins ab', async () => {
    const user = await createUser()
    const task = await createTask({ title: 'Papiertonne rausstellen', type: 'once', dueDate: addDaysToDateString(todayString(), 1), isAutoGenerated: true })
    const res = await request(app).post(`/api/tasks/${task.id}/complete`).set(authHeader(user.id))
    expect(res.status).toBe(400)
    const count = await prisma.taskCompletion.count({ where: { taskId: task.id } })
    expect(count).toBe(0)
  })
})
