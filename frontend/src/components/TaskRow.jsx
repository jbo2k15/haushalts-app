...
const TaskRow = memo(function TaskRow({ task, onToggle }) {
  ...
  let rowBg = 'bg-white'
  if (task.isOverdue && !completed) rowBg = 'bg-red-50'
  ...
  return (
    ...
    >
      <div className={`w-5 h-5 rounded flex-shrink-0 border-2 flex items-center justify-center transition-colors ${
        completed ? 'bg-green-500 border-green-500' : task.isOverdue ? 'border-red-400' : 'border-gray-300'
      }`}>
        {completed && (
      ...
      <span className={`flex-1 text-sm ${completed ? 'line-through text-gray-400' : 'text-gray-800'}`}>
        {task.title}
