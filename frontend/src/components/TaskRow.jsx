...
const TaskRow = memo(function TaskRow({ task, onToggle }) {
  ...
  // Vorab-Info fuer einen gebankten Abfall-Termin (>= 3 Tage vor der Abholung,
  // noch nicht faellig): nur Hinweis, nicht abhakbar/klickbar, kein Checkbox.
  if (task.infoOnly) {
    return (
      <div
        className="flex items-center gap-3 px-3 py-3 border-b border-outline last:border-b-0 bg-surface-container select-none"
        data-testid="task-row-info"
        data-task-title={task.title}
      >
        <div className="w-5 h-5 shrink-0 flex items-center justify-center text-ink-faint" aria-hidden="true">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {task.isAutoGenerated && <Badge tone="neutral">Abfall</Badge>}
          <span className="text-xs text-ink-faint">Vorschau</span>
        </div>
      </div>
    )
  }
