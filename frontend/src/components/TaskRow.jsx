  ...
  return (
    ...
    >
      ...
      <span className={`flex-1 text-sm ${completed ? 'line-through text-gray-400 dark:text-gray-500' : 'text-gray-800 dark:text-gray-200'}`}>
        {task.title}
        {isMulti && count > 1 && (
          <span className="ml-1.5 text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-1.5 py-0.5 rounded-sm">
            ×{count}
          </span>
        )}
        {task.isOnce && task.dueDate && (
          <span className="ml-1.5 text-xs bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 px-1.5 py-0.5 rounded-sm">
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        {task.isAutoGenerated && (
          <span className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 px-1.5 py-0.5 rounded-sm">Abfall</span>
        )}
        {task.systemCompleted && (
          <span className="text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-1.5 py-0.5 rounded-sm">☔ Wegen Regen entfallen</span>
        )}
      ...
      {isMulti && completed && (
        <button
          ...
          className="shrink-0 w-11 h-11 flex items-center justify-center rounded-full border border-gray-300 dark:border-gray-500 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors text-lg font-semibold leading-none touch-manipulation"
        >
          −
        </button>
      )}
      {isDaily && !completed && !skipped && (
        <button
          ...
          className="shrink-0 w-11 h-11 flex items-center justify-center rounded-full border border-red-200 dark:border-red-800/60 bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors text-sm font-semibold leading-none touch-manipulation"
        >
          ✕
        </button>
      )}
