...
export default function TaskRow({ task, onToggle }) {
  ...
  else if (task.pinned) rowBg = 'bg-purple-50'
    ...
    >
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        {task.fixedWeekday != null && (
          <span className="text-xs text-purple-600 font-medium">{WEEKDAYS[task.fixedWeekday]}</span>
        )}
        {task.fixedDayOfMonth != null && (
          <span className="text-xs text-purple-600 font-medium">{task.fixedDayOfMonth}.</span>
        )}
        {task.isAutoGenerated && (
          <span className="text-xs bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded">Abfall</span>
        )}
        {task.overdueDay1 && !task.completed && !task.overdueDay2 && (
          <span className="text-xs text-orange-600">gestern</span>
        )}
        {task.overdueDay2 && !task.completed && (
          <span className="text-xs text-red-600">2 Tage</span>
        )}
      </div>
      {loading && <div className="w-4 h-4 border-2 border-purple-400 border-t-transparent rounded-full animate-spin flex-shrink-0" />}
    </div>
  )
}
